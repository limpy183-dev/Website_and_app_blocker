...
Notes: this directory **is** a git repo (default branch `main`). Most enforcement (hosts file, policies) only works when run elevated; without admin the engine logs warnings and degrades rather than crashing. Tests are designed to import and run on non-Windows too (see "Cross-platform" below) — Qt-based tests use the offscreen platform (`QT_QPA_PLATFORM=offscreen python -m pytest tests/`). The suite is 31 tests.
...
- **`hosts.py`** — writes domains into the Windows hosts file between `# >>> FocusFortress begin/end` markers, expanding each to `www./m./mobile./cdn.` etc. variants (both `127.0.0.1` and `::1`), then flushes DNS. Only bare domains; wildcards/paths are rejected here. When `settings.block_doh` is on, the engine also injects `blocking/doh.py`'s well-known DNS-over-HTTPS endpoints here (minus `settings.doh_allowlist`) so browsers can't tunnel around the hosts file.
